        ...
        try:
            ...
            if self._login():
                ...
                raise SessionExpiredError(
                    f"Automatic re-login failed during {func.__name__}.") from e
    return wrapper
...
class Scraper:
    ...
    def _login(self) -> bool:
        ...
        try:
            ...
            if response_data.get("X_WINTER_REDIRECT"):
                ...
                raise Exception(
                    f"Login failed. Server responded with: {response.text}")
            ...
            if e.response.status_code == 500 and e.request.url == LOGIN_URL:
                ...
                logging.error(
                    ...
                    f"{e.response.reason} for url: {e.request.url}")
    ...
    def get_classes(self, days_in_advance: int = 7) -> List[Dict[str, Any]]:
        ...
        for i in range(days_in_advance):
            ...
            classes_json_response = self._get_classes_for_single_date(
                target_date_str)
            classes_html = classes_json_response.get('@events')
            if classes_html:
                parsed_classes = self._parse_classes_from_html(
                    classes_html, target_date)
    ...
    def find_and_book_class(
        ...
        if target_time and class_name:
            logging.info(
                f"Attempting to book class like '{class_name}' at {target_time} on {target_date_str}")
        else:
            logging.info(
                f"Attempting to book '{class_name}' on {target_date_str}")
        ...
        for gym_class in gym_classes:
            ...
            if start_time_str != "N/A" and end_time_str != "N/A":
                try:
                    ...
                    logging.warning(
                        f"Could not parse time for duration calculation: {start_time_str} - {end_time_str}")

            class_date = target_date.strftime("%d/%m/%Y")

            remaining_spaces_tag = gym_class.find(
                'span', {'class': 'remaining'})
            available_spaces = int(remaining_spaces_tag.text.strip(
            )) if remaining_spaces_tag and remaining_spaces_tag.text.strip().isdigit() else 0
    ...
    def _parse_and_execute_booking(self,
        ...
        for gym_class in gym_classes:
            ...
            # Explicitly skip virtual classes
        ...
        if best_match_element and highest_score > 64:
            ...
            logging.info(
                f"Found best match for '{class_name}': '{title}' with score {highest_score}")
            ...
            if "X_OCTOBER_REDIRECT" in response.text:
                raise SessionExpiredError(
                    f"The {action_description} failed, possibly due to a stale session.")
    ...
    def find_and_cancel_booking(
            self, class_name: str, target_date_str: str, target_time: str,
            instructor_name: str = "") -> Dict[str, Any]:
        """Finds a specific class on a given date and cancels the booking."""
        logging.info(
            f"Attempting to cancel '{class_name}' at {target_time} on {target_date_str}")
        json_response = self._get_classes_for_single_date(target_date_str)
        classes_html = json_response.get('@events')
        if not classes_html:
            return {
    ...
    def _parse_and_execute_cancellation(
            self, classes_html: str, class_name: str, target_time: str,
        ...
        for gym_class in gym_classes:
            ...
            if class_name.lower() in title.lower() and start_time_str == target_time:
                ...
                if instructor_name:
                    ...
                    for p in p_tags:
                        if p.text.lower().strip().startswith('with '):
                            instructor_from_html = p.text.strip()[
                                5:].replace('.', '')
                            break

                    if instructor_name.lower() in instructor_from_html.lower():
                        logging.info(
                            f"Found matching class with instructor: {title}")
        ...
        if not button:
            return {
                "status": "error",
                "message": (
                    "You do not appear to be booked on this class, "
                    "so cancellation is not possible."
                )
            }

        handler = form.get('data-request')
        class_id_input = form.find('input', {'name': 'id'})
        timestamp_input = target_class_element.find(
            'input', {'name': 'timestamp'})

        if not (handler and class_id_input and timestamp_input and
        ...
        if "X_OCTOBER_REDIRECT" in response.text:
            raise SessionExpiredError(
                "Cancellation failed, possibly due to a stale session.")
    ...
    def get_my_bookings(self) -> List[Dict[str, str]]:
        ...
        if not bookings_container:
            logging.info(
                "Could not find 'upcoming_bookings' container on members page. It might be empty.")
            return []
