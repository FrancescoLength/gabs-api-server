...
def test_reset_failed_bookings_flow(memory_db):
    ...
    booking_id_stuck = database.add_auto_booking(
        "test_user", "Test Class 2", "12:00", "Tuesday", "Test Instructor")
    database.update_auto_booking_status(booking_id_stuck, "in_progress")

    # Booking 3: Failed but recent (should NOT be reset)
    booking_id_recent_fail = database.add_auto_booking(
        "test_user", "Test Class 3", "14:00", "Wednesday", "Test Instructor")
    one_hour_ago = int((datetime.now() - timedelta(hours=1)).timestamp())
    database.update_auto_booking_status(
        booking_id_recent_fail, "failed", last_attempt_at=one_hour_ago)
    ...
    updated_booking_recent = database.get_auto_booking_by_id(
        booking_id_recent_fail)
...
def test_refresh_sessions_success(memory_db, mocker):
    ...
    mock_sync.assert_called_once_with(
        username, [{"name": "Class", "date": "2025-01-01", "time": "10:00"}])
