...
def test_get_csrf_token_tag_dict_behavior(scraper_mock, mocker):
    ...
    # The code `if token_tag and isinstance(token_tag, dict):` is likely checking for raw dicts (maybe from JSON parsing mocked elsewhere?).
...
def test_get_csrf_token_exception(scraper_mock, mocker):
    scraper_mock.session.get.side_effect = requests.exceptions.RequestException("Net error")
...
def test_login_success(scraper_mock, mocker):
    mocker.patch.object(scraper_mock, '_get_csrf_token', return_value="fresh_token")
...
def test_find_and_book_class_waitlist(scraper_mock, mocker):
    ...
    result = scraper_mock.find_and_book_class("2025-01-01", "Test Class", "10:00")
...
def test_find_and_book_class_already_booked(scraper_mock, mocker):
    ...
    """
    mocker.patch.object(scraper_mock, '_get_classes_for_single_date', return_value={"@events": html})
    mocker.patch.object(scraper_mock, '_get_csrf_token', return_value="fresh") # Added mock
...
def test_find_and_cancel_booking_success(scraper_mock, mocker):
    ...
    result = scraper_mock.find_and_cancel_booking("Cancel Me", "2025-01-01", "10:00")
...
def test_get_my_bookings_redirect_login(scraper_mock, mocker):
    ...
    mocker.patch.object(scraper_mock, '_login', return_value=False) # Login fails so exception propagates
...
def test_get_my_bookings_parsing_error(scraper_mock, mocker):
    ...
    mock_logger_warning = mocker.patch('gabs_api_server.scraper.logging.warning')
    
    bookings = scraper_mock.get_my_bookings()
    
    assert len(bookings) == 0
    mock_logger_warning.assert_called_with("Could not parse booking string: Unparseable String Here")
...
def test_find_and_book_class_match_not_found(scraper_mock, mocker):
    ...
    mocker.patch.object(scraper_mock, '_get_csrf_token', return_value="fresh") # Added mock here too
