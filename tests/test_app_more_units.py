...
from gabs_api_server.app import debug_file_writer, debug_writer_queue, get_logs, get_status, app, admin_required
...
def test_debug_file_writer_logic(mocker):
    ...
    mock_queue.get.side_effect = [('/path/to/file.txt', 'content'), (None, None)]
    ...
    mock_open_func.assert_called_with('/path/to/file.txt', 'w', encoding='utf-8')
...
def test_get_logs_success(test_client, mocker):
    ...
    response = test_client.get('/api/admin/logs', headers={'Authorization': f'Bearer {admin_token}'})
...
def test_get_status_success(test_client, mocker):
    ...
    mocker.patch('gabs_api_server.app.requests.get', return_value=mock_response)

    response = test_client.get('/api/admin/status', headers={'Authorization': f'Bearer {admin_token}'})
    assert response.status_code == 200
...
def test_get_status_ngrok_error(test_client, mocker):
    ...
    mocker.patch('gabs_api_server.app.requests.get', side_effect=requests.exceptions.RequestException("Connection failed"))
    mock_logging_error = mocker.patch('gabs_api_server.app.logging.error')
