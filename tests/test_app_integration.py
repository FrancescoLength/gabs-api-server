...
def test_sync_live_bookings_add_and_delete(memory_db, mocker):
    ...
    scraped_bookings = [
        {"name": "New Class", "date": "Monday 1st January",
...
def test_sync_live_bookings_case_insensitivity(memory_db, mocker):
    ...
    scraped_bookings = [
        {"name": "Boxfit", "date": "Monday 1st January",
...
def test_sync_live_bookings_invalid_date_format(memory_db, mocker):
    ...
    scraped_bookings = [
        {"name": "Bad Date Class", "date": "Not a Date",
            "time": "10:00", "status": "Booked"}
    ]
...
def test_sync_live_bookings_full_booking_not_found(memory_db, mocker):
    ...
    scraped_bookings = [
        {"name": "Existing Class", "date": "Monday 1st January",
            "time": "10:00", "status": "Booked", "instructor": "John Doe"}
    ]

    # Mock database.add_live_booking to check if it's called
    mock_add_live_booking = mocker.patch(
        'gabs_api_server.database.add_live_booking')
    ...
    # Make next return None for the first call
    mocker.patch('gabs_api_server.app.next', side_effect=[None])
...
def test_admin_endpoints_access(client, mocker):
    ...
    endpoints_to_test = [
        ('/api/admin/auto_bookings',
         'gabs_api_server.database.get_all_auto_bookings', []),
        ('/api/admin/live_bookings',
         'gabs_api_server.database.get_all_live_bookings', []),
        ('/api/admin/push_subscriptions',
         'gabs_api_server.database.get_all_push_subscriptions', []),
        ('/api/admin/sessions', 'gabs_api_server.database.get_all_sessions', []),
        # status mocks requests.get
        ('/api/admin/status', None, {"status": "ok", "uptime": "123"})
    ]
...
def test_admin_endpoints_get_status_ngrok_failure(client, mocker):
    ...
    mocker.patch('gabs_api_server.app.requests.get',
                 side_effect=RequestException("Ngrok connection failed"))
    ...
    # Should still return 200 OK, but with no ssh_tunnel_command
...
def test_api_logout_success(client, mocker):
    ...
    mock_delete_session = mocker.patch(
        'gabs_api_server.database.delete_session', return_value=True)
...
def test_api_book_success(client, mocker):
    ...
    mocker.patch('gabs_api_server.database.load_session',
                 return_value=("encrypted_pw", {"cookies": {}}))
    mocker.patch('gabs_api_server.crypto.decrypt',
...
def test_api_book_missing_parameters(client, mocker):
    ...
    # Mock get_scraper_instance for login success
    mock_scraper = mocker.Mock()
    # Mock to_dict if it's called during save_session
    mock_scraper.to_dict.return_value = {}
    mocker.patch('gabs_api_server.app.get_scraper_instance',
                 return_value=mock_scraper)
...
def test_api_book_session_expired(client, mocker):
    ...
    mock_handle_session_expiration = mocker.patch(
