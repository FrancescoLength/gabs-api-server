...
def client(mocker):
    ...
    # Patch limiter
    mocker.patch('gabs_api_server.app.limiter.limit',
...
def test_schedule_auto_book_success(client, auth_headers, mocker):
    ...
    response = client.post('/api/schedule_auto_book',
...
def test_schedule_auto_book_error(client, auth_headers, mocker):
    mocker.patch('gabs_api_server.database.add_auto_booking',
...
def test_get_auto_bookings_success(client, auth_headers, mocker):
    mock_bookings = [
        (1, 'test_user', 'Class', '10:00', 'pending',
         'now', None, 0, 'Monday', 'John', None)
    ]
    mocker.patch('gabs_api_server.database.get_auto_bookings_for_user',
...
def test_cancel_auto_book_success(client, auth_headers, mocker):
    mocker.patch('gabs_api_server.database.cancel_auto_booking',
                 return_value=True)

    response = client.post('/api/cancel_auto_book',
                           headers=auth_headers, json={'booking_id': 1})
    assert response.status_code == 200
...
def test_cancel_auto_book_not_found(client, auth_headers, mocker):
    ...
    response = client.post('/api/cancel_auto_book',
                           headers=auth_headers, json={'booking_id': 999})
...
def test_subscribe_push_success(client, auth_headers, mocker):
    ...
    response = client.post('/api/subscribe-push',
                           headers=auth_headers, json={'endpoint': 'url'})
    assert response.status_code == 201
